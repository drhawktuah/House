...
public sealed class HelpModule : BaseCommandModule
{
    public required CommandsNextExtension CommandsNext { get; set; }
    public required InteractivityExtension Interactivity { get; set; }
    public required HouseFuzzyMatchingService MatchingService { get; set; }
    ...
    public async Task HelpAsync(CommandContext context, [RemainingText] string? query = null)
    {
        HouseHelpFormatter helpFormatter = new(context);
        ...
        else
        {
            ...
            {
                var results = MatchingService.GetResults(query);
                if (!results.Any())
                {
                    await context.RespondAsync($"no command named `'{query}'` was found, and no similar commands could be suggested");
                    return;
                ...
                var embed = await MatchingService.ToDiscordEmbed(query, results);
                await context.RespondAsync(embed);
            }
        }
    }
    ...
    /*
    private static DiscordMessageBuilder CreateHelpMessage(IReadOnlyList<Page> pages)
    {
        DiscordMessageBuilder messageBuilder = new();
        ...
        for (int i = 0; i < pages.Count && i < EmbedsPerMessage; i++)
        {
            if (pages[i].Embed != null)
            {
                messageBuilder.AddEmbed(pages[i].Embed);
            }
        }
